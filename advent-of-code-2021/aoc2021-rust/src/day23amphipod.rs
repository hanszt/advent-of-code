...
impl State {
    ...
    fn room_to_hallway(&self, room_idx: usize, y: usize, hallway_x: usize) -> Option<usize> {
        if entry_at_hallway_x(hallway_x).is_some() {
            // We can't stop at a hallway entry.
            None
    ...
    /// of that move.
    fn moves(&self) -> Vec<(State, usize)> {
        let mut moves = Vec::new();
        for i in 0..11 {
            if let Some((cost, room, y)) = self.hallway_to_dest(i, self.bottom) {
                let mut n = *self;
...
fn solve(input: &str, to_state: impl Fn(&[char], &[char]) -> State) -> usize {
    ...
    while let Some((_, Reverse(cost), state)) = queue.pop() {
        ...
        for (next, move_cost) in state.moves() {
...
\n";
    ...
    #[test]
    fn example1() {
        assert_eq!(super::part_1(EXAMPLE_INPUT), 12_521);
    }

    #[test]
    fn part_1() {
        assert_eq!(super::part_1(include_str!("../../input/day23.txt")), 14_348);
    }

    #[test]
    fn example2() {
        assert_eq!(super::part_2(EXAMPLE_INPUT), 44_169);
    }

    #[test]
    fn part_2() {
        assert_eq!(super::part_2(include_str!("../../input/day23.txt")), 40_954);
    }
}
