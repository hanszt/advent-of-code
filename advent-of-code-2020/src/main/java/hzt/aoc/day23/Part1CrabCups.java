...
public class Part1CrabCups extends Day23Challenge {
    ...
    protected long calculateAnswer(final IntList cups) {
        final var lowestCupLabel = cups.min();
        final var highestCupLabel = cups.max();
        final var mutableCups = cups.toMutableList();
        var indexCurrent = 0;
        for (var i = 0; i < NR_OF_MOVES; i++) {
            final var currentCupLabel = mutableCups.get(indexCurrent);
            final var threePickedUpCups = listPickedUpCups(indexCurrent, mutableCups);
            mutableCups.removeAll(threePickedUpCups);
            final var targetCupLabel = determineTargetCupLabel(currentCupLabel, lowestCupLabel, highestCupLabel, mutableCups);
            final var targetIndex = getIndexByLabel(targetCupLabel, mutableCups);
    ...
    private long arrangeInOrder(final IntList cups) {
        final var indexCupOne = getIndexByLabel(CUP_ONE_LABEL, cups);
        final var stringBuilder = new StringBuilder();
        for (var i = 0; i < cups.size() - 1; i++) {
